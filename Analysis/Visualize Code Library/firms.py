...
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    ...
    # 等值划分
    num_bins = num_bins
    Stfmean_range_all_years = [min_firms_Stfmean_all_years + (max_firms_Stfmean_all_years - min_firms_Stfmean_all_years) * i / num_bins for i in range(num_bins + 1)]
    gdf['bin_Firms'] = pd.cut(gdf[Stfmean_column_name], bins=Stfmean_range_all_years, labels=range(num_bins), duplicates='drop')
