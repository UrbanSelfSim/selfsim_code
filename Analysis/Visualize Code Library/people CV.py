...
output_folder = os.path.join(os.getcwd(), "people CV街道图中转")
...
for year in years:
    ...
    if os.path.isfile(csv_file_path): 
        ...
        # 导出 CV 列为 1 的数据
        cv_df = df[df['CV'] == 1]
        cv_output_path = os.path.join(output_folder1, f"people CV{year}year.csv")
...
for year in years:
    csv_file_path = f'csv中转/people CV{year}year.csv'
    #print(csv_file_path)
    if os.path.isfile(csv_file_path): 
        df = pd.read_csv(csv_file_path)
        ...
        output_shapefile = os.path.join(output_folder, f"people CV {year} year.shp")
...
input_folder = "people CV街道图中转"  
output_folder = "people CV街道图中转/第一步图像相交结果"  
...
for filename in os.listdir(input_folder):
    if filename.endswith(".shp"):  
        ...
        result_gdf['CVmean'] = result_gdf['people-ID'] / result_gdf['AREA'] *1000
...
shp_folder = "people CV街道图中转\第一步图像相交结果"  
gdf_list = []
...
for gdf in gdf_list:
    CVmean_column_name = "CVmean"  
    if CVmean_column_name in gdf.columns:
        rounded_values = gdf[CVmean_column_name].apply(lambda x: round(x))
        all_CVple_CVmean.extend(rounded_values)

max_CVple_CVmean_all_years = max(all_CVple_CVmean)
min_CVple_CVmean_all_years = min(all_CVple_CVmean)
#print(all_CVple_CVmean)

num_bins = 5

# 等值划分
num_bins = num_bins
CVmean_range_all_years = [min_CVple_CVmean_all_years + (max_CVple_CVmean_all_years - min_CVple_CVmean_all_years) * i / num_bins for i in range(num_bins + 1)]  # 不要改变这一行
#CVmean_range_all_years = [min_CVple_CVmean_all_years + (max_CVple_CVmean_all_years - min_CVple_CVmean_all_years) * i / num_bins for i in range(num_bins + 1)]
gdf['bin_CV'] = pd.cut(gdf[CVmean_column_name], bins=CVmean_range_all_years, labels=range(num_bins), duplicates='drop')
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    ...
    gdf.plot(column=CVmean_column_name, ax=ax, cmap=cmap, legend=False)  
    base_map.plot(ax=ax, color='none', edgecolor='black', linewidth=2)

    # 自定义块状图例
    for j in range(num_bins):
        ax.fill_between([], [], color=cmap(j), label=f'{int(CVmean_range_all_years[j])}-{int(CVmean_range_all_years[j+1])}')

    # 创建独立的图例对象并添加到子图中
    legend_elements = [Patch(facecolor=cmap(j), edgecolor='black', label=f'{int(CVmean_range_all_years[j])}-{int(CVmean_range_all_years[j+1])}')
                       for j in range(num_bins)]
...
# 几何划分
nonzero_values = [value for value in all_CVple_CVmean if value != 0]
...
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    ...
    cmap = plt.get_cmap('Reds', len(percentiles_all) - 1)

    # 将数据为0的地块单独设置为白色
    gdf_zero = gdf[gdf[CVmean_column_name] == 0]
    gdf_nonzero = gdf[gdf[CVmean_column_name] != 0]

    gdf_nonzero['bin_CV'] = pd.cut(gdf_nonzero[CVmean_column_name], bins=percentiles_all, labels=range(len(percentiles_all) - 1))

    # 绘制数据为0的地块，颜色设置为白色
    gdf_zero.plot(ax=ax, color='white', legend=False)
    gdf_nonzero.plot(column='bin_CV', ax=ax, cmap=cmap, legend=False)  
    base_map.plot(ax=ax, color='none', edgecolor='black', linewidth=2)
