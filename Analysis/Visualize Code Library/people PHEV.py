...
#该py文件只对people历年文件中的EV PHEV PHEV列进行筛选出图处理，不会对原csv备份移动
#因此该代码只有出图功能

#请在 街道shp加边框.py 前使用，单独后使用记得清理 原csv文件

#若EV PHEV PHEV列中其中某列全部为0则会显示ValueError: max() arg is an empty sequence
...
output_folder = os.path.join(os.getcwd(), "people PHEV街道图中转")
...
for year in years:
    ...
    if os.path.isfile(csv_file_path): 
        ...
        # 导出 PHEV 列为 1 的数据
        PHEV_df = df[df['PHEV'] == 1]
        PHEV_output_path = os.path.join(output_folder1, f"people PHEV{year}year.csv")
        PHEV_df.to_csv(PHEV_output_path, index=False)
...
for year in years:
    csv_file_path = f'csv中转/people PHEV{year}year.csv'
    #print(csv_file_path)
    if os.path.isfile(csv_file_path): 
        df = pd.read_csv(csv_file_path)
        ...
        output_shapefile = os.path.join(output_folder, f"people PHEV {year} year.shp")
...
input_folder = "people PHEV街道图中转"  
output_folder = "people PHEV街道图中转/第一步图像相交结果"  
...
for filename in os.listdir(input_folder):
    if filename.endswith(".shp"):  
        ...
        result_gdf['PHEVmean'] = result_gdf['people-ID'] / result_gdf['AREA'] *1000
...
shp_folder = "people PHEV街道图中转\第一步图像相交结果"  
gdf_list = []
...
all_PHEVple_PHEVmean = []

for gdf in gdf_list:
    PHEVmean_column_name = "PHEVmean"  
    if PHEVmean_column_name in gdf.columns:
        rounded_values = gdf[PHEVmean_column_name].apply(lambda x: round(x))
        all_PHEVple_PHEVmean.extend(rounded_values)

max_PHEVple_PHEVmean_all_years = max(all_PHEVple_PHEVmean)
min_PHEVple_PHEVmean_all_years = min(all_PHEVple_PHEVmean)
#print(all_PHEVple_PHEVmean)

num_bins = 5
# 等值划分
num_bins = num_bins
PHEVmean_range_all_years = [min_PHEVple_PHEVmean_all_years + (max_PHEVple_PHEVmean_all_years - min_PHEVple_PHEVmean_all_years) * i / num_bins for i in range(num_bins + 1)]# 不要改变这一行
#gdf['bin_PHEV'] = pd.cut(gdf[PHEVmean_column_name], bins=num_bins, labels=False, duplicates='drop')
gdf['bin_PHEV'] = pd.cut(gdf[PHEVmean_column_name], bins=PHEVmean_range_all_years, labels=range(num_bins), duplicates='drop')
...
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    ...
    gdf.plot(column=PHEVmean_column_name, ax=ax, cmap=cmap, legend=False)  
    base_map.plot(ax=ax, color='none', edgecolor='black', linewidth=2)

    # 自定义块状图例
    for j in range(num_bins):
        ax.fill_between([], [], color=cmap(j), label=f'{round(PHEVmean_range_all_years[j],3)}-{round(PHEVmean_range_all_years[j+1],3)}')

    # 创建独立的图例对象并添加到子图中
    legend_elements = [Patch(facecolor=cmap(j), edgecolor='black', label=f'{round(PHEVmean_range_all_years[j],3)}-{round(PHEVmean_range_all_years[j+1],3)}')
                       for j in range(num_bins)]
...
# 几何划分
nonzero_values = [value for value in all_PHEVple_PHEVmean if value != 0]
...
for i, (shp_file, gdf) in enumerate(zip(shp_files, gdf_list)):
    ...
    cmap = plt.get_cmap('YlOrBr', len(percentiles_all) - 1)

    # 将数据为0的地块单独设置为白色
    gdf_zero = gdf[gdf[PHEVmean_column_name] == 0]
    gdf_nonzero = gdf[gdf[PHEVmean_column_name] != 0]

    gdf_nonzero['bin_PHEV'] = pd.cut(gdf_nonzero[PHEVmean_column_name], bins=percentiles_all, labels=range(len(percentiles_all) - 1))

    gdf_zero.plot(ax=ax, color='white', legend=False)
    gdf_nonzero.plot(column='bin_PHEV', ax=ax, cmap=cmap, legend=False)  
    base_map.plot(ax=ax, color='none', edgecolor='black', linewidth=2)
